...
import CarnaticScalesPattern from "./CarnaticScalesPattern";
...
const CarnaticScalesMulti: React.FC<ScalesMultiProps> = ({ patterns, scalesPatternType, ragasPatterns }) => {
    ...
    <div>
      {instances.map(({ id }) => (
        <div key={id} style={{ marginBottom: 32, border: selectedId === id ? '2px solid #1976d2' : '2px solid #eee', borderRadius: 8, background: selectedId === id ? '#e3f2fd' : '#fff', boxShadow: selectedId === id ? '0 2px 8px #90caf9' : '0 2px 8px #eee', padding: 14, position: 'relative' }}>
          ...
          <CarnaticScalesPattern zoom={zoom} patterns={patterns} ragasPatterns={ragasPatterns} />
        </div>
      ))}
      <button
        onClick={handleAdd}
...
export default CarnaticScalesMulti;
