...
const ScalesPattern: React.FC<ScalesPatternProps> = ({ zoom = 100, patterns, scalesPatternType, ragasPatterns, addScratchPadItem }) => {

  // Filter out header rows like ["--- Group ---", ...] before generating patterns
  const isHeader = (name: any) => typeof name === 'string' && name.startsWith('---') && name.endsWith('---');
  let CHORDS_PATTERNS = patterns
    .filter((row: any) => row && !isHeader(row[0]))
    .map(generateChordPattern);
